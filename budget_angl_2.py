...
class ExpenseTrackerApp(tk.Tk):
    def __init__(self):
        ...
        self.categories = ["groceries", "entertainment", "travels", "clothes", 
                           "rent", "donation", "health", "gifts", "savings", "other"]
    ...
    def update_total_expenses(self):
        ...
        self.total_expenses_label.config(text=f"Total Expenses per session: {total}")
    ...
    def show_expenses(self):
        ...
        for expense in expenses:
            self.expenses_text.insert(tk.END, f"[{expense[3]}]  Amount: {expense[1]} $     Category - {expense[2]}     COMMENT: {expense[4]}\n")

    def show_monthly_chart(self):
        if self.monthly_chart_frame.winfo_ismapped():
            # Delete previous expense schedule
    ...
    def create_widgets(self):
        title_label = tk.Label(self, text="BUDGET TRACKER", fg='#00f', font=("Comic Sans MS",  26,"bold"), underline=6)
        ...
        # Total Expenses
        self.total_expenses_label = tk.Label(self, text="Total Expenses per session: 0", fg='darkblue', font=("Comic Sans MS", 12, "bold"),)
