...
class ExpenseTrackerApp(tk.Tk):
    ...
    def show_monthly_info(self):
        if self.monthly_info_label.winfo_ismapped():
            self.monthly_info_label.pack_forget()
        else:
            ...
            self.monthly_info_label.config(text=f"Monthly Info:\n{dict(zip(months, expenses))}")
            self.monthly_info_label.pack()

    def show_yearly_info(self):
        if self.yearly_info_label.winfo_ismapped():
            self.yearly_info_label.pack_forget()
        else:
            self.c.execute("SELECT strftime('%Y', date) AS year, SUM(amount) FROM expenses GROUP BY year")
            data = self.c.fetchall()
            years = [row[0] for row in data]
            expenses = [row[1] for row in data]
            self.yearly_info_label.config(text=f"Yearly Info:\n{dict(zip(years, expenses))}")
            self.yearly_info_label.pack()

    def show_monthly_chart(self):
        self.c.execute("SELECT category, SUM(amount) FROM expenses GROUP BY category")
        data = self.c.fetchall()
    ...
    def create_widgets(self):
        ...
        # Monthly and Yearly Info
        self.monthly_info_label = tk.Label(self, text="")
        self.yearly_info_label = tk.Label(self, text="")

        monthly_info_button = tk.Button(self, text="Show Monthly Info", command=self.show_monthly_info)
        monthly_info_button.pack(pady=5)

        yearly_info_button = tk.Button(self, text="Show Yearly Info", command=self.show_yearly_info)
        yearly_info_button.pack(pady=5)
